...
Prend en params (le nom de dossier,
puis si existe deja pas besoin de le creer de nouveau)
'''
...
for categorie in list_categories:
    ...
    # print(url_books)
    # Ici je donne le chemin des images de chaque categorie
    images_path = f'./product_images/{categorie}'
    '''
    Boucle dans ma boucle pour recuperer
    le lien de l'image de chaque livre des categories.
    '''
    for i in range(len(url_books)):
        # je met ici l'index "i" pour indexer les images
        url_book = url_books[i]
        book_data = get_infos_book(url_book)
        url_img = book_data["url_image"]
        image_path = images_path + f'/{url_book.split("/")[4]}.jpg'
        urllib.request.urlretrieve(url_img, image_path)
        print(f"img {i+1}: download successful")


'''
Le module os nous permet (entre autres) de travailler
avec les fichiers et les répertoires.
Ce module interargi avec les fonctionnalites du systeme
d'exploitation et accede aux informations.
Il possede des functions que l'on peut appeler
ainsi que des valeurs normales.
'''

'''
urllib est un module de gestion des urls
Le module urllib.request definit des fonctions et
des classes qui aident a ouvrir des url.
'''
...
Paramètre :
1 - path : Un objet semblable à un chemin représentant
un chemin de système de fichiers.
Un objet de type chemin est soit une string,
soit un objet d’octets représentant un chemin.

2 - mode (facultatif) :
Valeur entière représentant le mode
du répertoire nouvellement créé.
Si ce paramètre est omis,
la valeur par défaut Oo777 est utilisée.

3 - exist_ok (optionnel) :
Une valeur par défaut False est utilisée pour ce paramètre.
Si le répertoire cible existe déjà,
une OSError est levée si sa valeur est False sinon non.
